...
class PerfilController {
    ...
    amigos2(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            const amistades = yield database_1.default.query('SELECT colaborador.nombre,colaborador.idColaborador,colaborador.fotoPerfil,colaborador.email FROM colaborador INNER JOIN (SELECT idColaborador2 FROM amigo WHERE idColaborador1 =? AND aceptado = 1) AS amigos ON amigos.idColaborador2 = colaborador.idColaborador', [id]);
            const amistades2 = yield database_1.default.query('SELECT colaborador.nombre,colaborador.idColaborador,colaborador.fotoPerfil,colaborador.email FROM colaborador INNER JOIN (SELECT idColaborador1 FROM amigo WHERE idColaborador2 = ? AND aceptado = 1) AS amigos ON amigos.idColaborador1 = colaborador.idColaborador', [id]);
            if (amistades.length >= 1) {
                if (amistades2.length >= 1) {
                    const allAmistades = amistades.concat(amistades2);
                    res.json(allAmistades);
